"""
Training script for user baseline anomaly detection model.
"""
import pandas as pd
import joblib
...
from src.models.baseline_model import create_baseline_model, get_model_config
from src.features.baseline_features import get_baseline_feature_names
...
# Constants
RANDOM_STATE = 42
DATA_PATH = Path(__file__).resolve().parent.parent.parent / "data" / "raw" / "user_baseline_synthetic_10000.csv"
WEIGHTS_DIR = Path(__file__).resolve().parent.parent.parent / "weights"
MODEL_NAME = "baseline_model_v1.pkl"
...
def load_data(data_path: Path) -> Tuple[pd.DataFrame, pd.Series]:
    ...
    # Extract features
    features = get_baseline_feature_names()
    X = df[features]
    y = df["baseline_anomaly_flag"]
...
def train_model(X_train: pd.DataFrame, y_train: pd.Series) -> tuple:
    """
    Train the user baseline anomaly detection model.
    ...
    pipeline = create_baseline_model(**{k: v for k, v in config.items() if k != "features"})
